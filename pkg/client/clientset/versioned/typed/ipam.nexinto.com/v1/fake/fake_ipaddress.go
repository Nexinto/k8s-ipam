/*
Copyright 2018 Nexinto

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	ipam_nexinto_com_v1 "github.com/Nexinto/k8s-ipam/pkg/apis/ipam.nexinto.com/v1"
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeIpAddresses implements IpAddressInterface
type FakeIpAddresses struct {
	Fake *FakeIpamV1
	ns   string
}

var ipaddressesResource = schema.GroupVersionResource{Group: "ipam.nexinto.com", Version: "v1", Resource: "ipaddresses"}

var ipaddressesKind = schema.GroupVersionKind{Group: "ipam.nexinto.com", Version: "v1", Kind: "IpAddress"}

// Get takes name of the ipAddress, and returns the corresponding ipAddress object, and an error if there is any.
func (c *FakeIpAddresses) Get(name string, options v1.GetOptions) (result *ipam_nexinto_com_v1.IpAddress, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(ipaddressesResource, c.ns, name), &ipam_nexinto_com_v1.IpAddress{})

	if obj == nil {
		return nil, err
	}
	return obj.(*ipam_nexinto_com_v1.IpAddress), err
}

// List takes label and field selectors, and returns the list of IpAddresses that match those selectors.
func (c *FakeIpAddresses) List(opts v1.ListOptions) (result *ipam_nexinto_com_v1.IpAddressList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(ipaddressesResource, ipaddressesKind, c.ns, opts), &ipam_nexinto_com_v1.IpAddressList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &ipam_nexinto_com_v1.IpAddressList{}
	for _, item := range obj.(*ipam_nexinto_com_v1.IpAddressList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested ipAddresses.
func (c *FakeIpAddresses) Watch(opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(ipaddressesResource, c.ns, opts))

}

// Create takes the representation of a ipAddress and creates it.  Returns the server's representation of the ipAddress, and an error, if there is any.
func (c *FakeIpAddresses) Create(ipAddress *ipam_nexinto_com_v1.IpAddress) (result *ipam_nexinto_com_v1.IpAddress, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(ipaddressesResource, c.ns, ipAddress), &ipam_nexinto_com_v1.IpAddress{})

	if obj == nil {
		return nil, err
	}
	return obj.(*ipam_nexinto_com_v1.IpAddress), err
}

// Update takes the representation of a ipAddress and updates it. Returns the server's representation of the ipAddress, and an error, if there is any.
func (c *FakeIpAddresses) Update(ipAddress *ipam_nexinto_com_v1.IpAddress) (result *ipam_nexinto_com_v1.IpAddress, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(ipaddressesResource, c.ns, ipAddress), &ipam_nexinto_com_v1.IpAddress{})

	if obj == nil {
		return nil, err
	}
	return obj.(*ipam_nexinto_com_v1.IpAddress), err
}

// Delete takes name of the ipAddress and deletes it. Returns an error if one occurs.
func (c *FakeIpAddresses) Delete(name string, options *v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(ipaddressesResource, c.ns, name), &ipam_nexinto_com_v1.IpAddress{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeIpAddresses) DeleteCollection(options *v1.DeleteOptions, listOptions v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(ipaddressesResource, c.ns, listOptions)

	_, err := c.Fake.Invokes(action, &ipam_nexinto_com_v1.IpAddressList{})
	return err
}

// Patch applies the patch and returns the patched ipAddress.
func (c *FakeIpAddresses) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *ipam_nexinto_com_v1.IpAddress, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(ipaddressesResource, c.ns, name, data, subresources...), &ipam_nexinto_com_v1.IpAddress{})

	if obj == nil {
		return nil, err
	}
	return obj.(*ipam_nexinto_com_v1.IpAddress), err
}
